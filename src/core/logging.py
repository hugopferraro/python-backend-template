...
- Non-dev: structured JSON output (for log aggregation via Loki/Grafana)
"""
import json
import logging
import sys
...
# Context variable for request-scoped data (e.g. request_id, user_id, action)
...
def set_log_context(**kwargs) -> None:
    ...
    Every subsequent log line in this request will include these fields.

    Usage:
        set_log_context(user_id=str(user.id), action="create")
    """
...
class ColoredFormatter(Formatter):
    """
    Human-readable formatter with ANSI colors for development.
    """
...
class JSONFormatter(Formatter):
    ...
    Outputs one JSON object per log line, making it parseable by Loki/Promtail.
    Includes any request-scoped context (e.g. request_id) automatically.
    """
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Merge request-scoped context (request_id, etc.)
        ctx = request_context.get()
        if ctx:
...
def configure_third_party_loggers(log_level: int) -> None:
    ...
    # Uvicorn logging - suppress access log (we handle it in the request middleware)
    ...
    # Passlib logging - suppress warnings
    passlib_logger = logging.getLogger("passlib")
    passlib_logger.handlers.clear()
    passlib_logger.propagate = True
